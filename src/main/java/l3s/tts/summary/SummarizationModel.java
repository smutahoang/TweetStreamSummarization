...
public class SummarizationModel {
	...
	protected List<Node> affectedNodes;
	protected AliasSample sampler;
	...
	public SummarizationModel() {
		...
		affectedNodes = new ArrayList<Node>();
		candidates = new ArrayList<Candidate>();
		sampler = new AliasSample(new Random());
	}
	...
	public void addNewTweet(Tweet tweet, int index) {
		...
		for (int i = 0; i < tokens.size(); i++) {
			...
			if (wordNodeMap.containsKey(tokens.get(i).token + "/" + tokens.get(i).tag.toLowerCase())) {
				isCurNodeNew = false;

				curNode = wordNodeMap.get(tokens.get(i).token + "/" + tokens.get(i).tag.toLowerCase());
				curNode.addNewTweetPosPair(index, i);

			} else { // if graph doesnt contain this node
				curNode = new Node(); // create a new node
				curNode.setNodeName(tokens.get(i).token + "/" + tokens.get(i).tag.toLowerCase());
