...
public class Sumblr {
	...
	public Sumblr(TweetStream _stream, long _refTime) {
		stream = _stream;
		refTime = _refTime;
		nTweets = 0;
		currentTime = 0;
	...
	private TCV chooseCluster(Tweet tweet) {
		...
		if (similarity < Configure.BETA * cCluster.getAvgSimilarity())
			return cCluster;
		else
			return null;
	}
	...
	private void mergeClusters() {
		if (clusters.size() < Configure.MERGE_TRIGGER) {
			return;
		}
		...
		while (queue.size() > 0) {
			if (nRemovedClusters >= clusters.size() * (1 - Configure.MERGE_THRESHOLD)) {
				return;
			}
			nRemovedClusters++;
